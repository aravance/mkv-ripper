...
func TestIngest(t *testing.T) {
	...
	ingester := LocalIngester{&url.URL{Path: testdir}}
...
func TestIngestNoShafile(t *testing.T) {
	...
	statMkvFile(t)
	compareShaFile(t, `c3ab8ff13720e8ad9047dd39466b3c8974e592c2fa383d4a3960714caef0c4f2  bar (1989)/bar (1989) - 1080p.mkv`)
...
func createShaFile(t *testing.T) string {
	shafile := fmt.Sprintf("%s/Movies.sha256", testdir)
	if err := os.WriteFile(shafile, []byte(shafileContent), 0644); err != nil {
		t.Fatalf("errror writing file '%s': %v", shafile, err)
	}
	return shafile
}

func statMkvFile(t *testing.T) {
